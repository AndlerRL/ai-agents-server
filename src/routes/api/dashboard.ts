...
export const dashboardDataRoutes = new Elysia({ prefix: '/v1/dashboard' })
  .get(
    '/agents',
    (context) => {
      try {
  ...
  .get(
        ...
        return {
          ...
          data: {
            ...
            agents: {
              total: agents.length,
              byStatus: {
                processing: agents.filter((a) => a.status === 'processing')
                  .length,
                idle: agents.filter((a) => a.status === 'idle').length,
                error: agents.filter((a) => a.status === 'error').length,
              },
              byProvider: {
                openai: agents.filter((a) => a.model.provider === 'openai')
                  .length,
                custom: agents.filter((a) => a.model.provider === 'custom')
                  .length,
                anthropic: agents.filter(
                  (a) => a.model.provider === 'anthropic'
                ).length,
              },
              byModel: agents.reduce(
                (acc, agent) => {
    ...
    },
    {
      detail: {
        ...
        description:
          'Get comprehensive statistics including all features (server, agents, webhooks, MCP)',
      },
    }
  )
  ...
  .get(
    ...
    (context) => {
      try {
        ...
        const openaiAgents = allAgents.filter(
          (a) => a.model.provider === 'openai'
        );
        ...
        return {
          ...
          data: {
            ...
            agents: {
              ...
              active: openaiAgents.filter((a) => a.status === 'processing')
                .length,
              idle: openaiAgents.filter((a) => a.status === 'idle').length,
              byModel: openaiAgents.reduce(
                (acc, agent) => {
                  const model = agent.model.model;
                  acc[model] = (acc[model] || 0) + 1;
                  return acc;
            ...
            usage: {
              ...
              avgResponseTime: stateSnapshot.statistics.averageResponseTime,
              totalRequests: openaiAgents.reduce(
                (sum, a) => sum + ((a as any).requestCount || 0),
                0
              ),
            },
            ...
            tools: {
              total: [
                ...new Set(
                  openaiAgents.flatMap((a) => a.tools.map((t) => t.name))
                ),
              ].length,
        ...
        return {
          ...
          data: {
            ...
            agents: {
              ...
              active: customAgents.filter((a) => a.status === 'processing')
                .length,
  ...
  .get(
    ...
    (context) => {
      try {
        ...
        const stateSnapshot = state.getStateSnapshot();
        const allAgents = Array.from(stateSnapshot.agents.values());
        const mcpAgents = allAgents.filter(
          (a) => (a as any).mcpServers?.length > 0
        );
          ...
          data: {
            ...
            agents: {
              ...
              avgServersPerAgent:
                mcpAgents.length > 0
                  ? mcpAgents.reduce(
                      (sum, a) => sum + ((a as any).mcpServers?.length || 0),
                      0
                    ) / mcpAgents.length
                  : 0,
            },
    ...
    },
    {
      detail: {
        ...
        description:
          'Get detailed statistics for MCP servers, tools, and MCP-enabled agents',
      },
    }
  )
        ...
        return {
          ...
          data: {
            tools: tools.map((tool: any) => ({
              ...
              parameterCount: Object.keys(tool.parameters.properties || {})
                .length,
            })),
            categories,
            summary: {
