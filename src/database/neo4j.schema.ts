...
export interface DocumentRelationship extends Neo4jRelationship {
  type: 'CITES' | 'CITED_BY' | 'RELATED_TO' | 'PART_OF' | 'CONTAINS' | 'DERIVED_FROM' | 'SUPERSEDES' | 'VERSION_OF'
}

// Chunk relationships
export interface ChunkRelationship extends Neo4jRelationship {
  type: 'FOLLOWS' | 'PRECEDES' | 'SIMILAR_TO' | 'CONTAINS_CONCEPT' | 'MENTIONS_ENTITY' | 'BELONGS_TO_TOPIC' | 'EXPLAINS' | 'CONTRADICTS'
}
...
export interface EntityRelationship extends Neo4jRelationship {
  type: 'IS_A' | 'PART_OF' | 'RELATED_TO' | 'WORKS_FOR' | 'LOCATED_IN' | 'CREATED_BY' | 'INFLUENCES' | 'DEPENDS_ON' | 'IMPLEMENTS' | 'USES'
}

// Concept relationships
export interface ConceptRelationship extends Neo4jRelationship {
  type: 'SUBCONCEPT_OF' | 'PREREQUISITE_FOR' | 'APPLIES_TO' | 'CONTRADICTS' | 'SUPPORTS' | 'EXAMPLE_OF' | 'GENERALIZES'
}

// Topic relationships
export interface TopicRelationship extends Neo4jRelationship {
  type: 'SUBTOPIC_OF' | 'RELATED_TOPIC' | 'COVERS_CONCEPT' | 'REQUIRES_KNOWLEDGE' | 'BUILDS_ON' | 'APPLIED_IN'
}
...
export interface GraphRetrievalResult extends Neo4jResult {
  nodes: (Neo4jDocument | Neo4jChunk | Neo4jEntity | Neo4jConcept | Neo4jTopic)[]
