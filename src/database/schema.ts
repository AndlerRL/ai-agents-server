...
// @ts-expect-error: When using default schema (public) it is not required to specify...
...
export const documents = publicSchema.table(
  'documents',
  {
...
export const documentChunks = publicSchema.table(
  'document_chunks',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    keywordVector: jsonb('keyword_vector')
  ...
  (table) => [
    ...
    index('chunks_embedding_ivf_idx').using(
      'ivfflat',
      table.embedding.op('vector_cosine_ops')
    ),
...
export const knowledgeGraph = publicSchema.table(
  'knowledge_graph',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const graphRelations = publicSchema.table(
  'knowledge_graph_relations',
  {
    id: uuid('id').primaryKey().defaultRandom(),

    sourceEntityId: text('source_entity_id')
      .references(() => knowledgeGraph.entityId, { onDelete: 'cascade' })
      .notNull(),
...
export const graphDocuments = graphPublicSchema.table(
  'documents',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    title: text('title').notNull(),
    ...
    outgoingRelationshipCount: integer('outgoing_relationship_count').default(
      0
    ),
    incomingRelationshipCount: integer('incoming_relationship_count').default(
      0
    ),
...
export const graphChunks = graphPublicSchema.table(
  'chunks',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    documentId: uuid('document_id')
      .references(() => graphDocuments.id, { onDelete: 'cascade' })
      .notNull(),
    ...
    semanticRelationshipCount: integer('semantic_relationship_count').default(
      0
    ),
  ...
  (table) => [
    ...
    index('graph_chunks_embedding_cosine_idx').using(
      'hnsw',
      table.embedding.op('vector_cosine_ops')
    ),
...
export const graphEntities = graphPublicSchema.table(
  'entities',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (table) => [
    ...
    index('graph_entities_embedding_idx').using(
      'hnsw',
      table.embedding.op('vector_cosine_ops')
    ),
...
export const graphRelationships = graphPublicSchema.table(
  'relationships',
  {
    id: uuid('id').primaryKey().defaultRandom(),

    sourceEntityId: text('source_entity_id')
      .references(() => graphEntities.entityId, { onDelete: 'cascade' })
      .notNull(),
    targetEntityId: text('target_entity_id')
      .references(() => graphEntities.entityId, { onDelete: 'cascade' })
      .notNull(),
  ...
  (table) => [
    ...
    index('graph_rels_composite_idx').on(
...
export const databaseSyncLog = graphPublicSchema.table(
  'database_sync_log',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const retrievalSessions = publicSchema.table(
  'retrieval_sessions',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const retrievalResults = publicSchema.table(
  'retrieval_results',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    sessionId: uuid('session_id')
      .references(() => retrievalSessions.id, { onDelete: 'cascade' })
      .notNull(),
    ...
    entityId: text('entity_id').references(() => knowledgeGraph.entityId, {
      onDelete: 'cascade',
    }),
...
export const embeddingShards = pgTable(
  'embedding_shards',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const documentShardMappings = pgTable(
  'document_shard_mappings',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    documentId: uuid('document_id')
      .references(() => documents.id, { onDelete: 'cascade' })
    ...
    shardId: uuid('shard_id')
      .references(() => embeddingShards.id, { onDelete: 'cascade' })
      .notNull(),
...
// ============================================================================
// AgentCanvas - Agentic Flow Management
// ============================================================================

// SDK types supported by the system
export const sdkTypeEnum = pgEnum('sdk_type', ['openai', 'vercel_ai']);
...
export const agents = publicSchema.table(
  'agents',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    type: agentTypeEnum('type').notNull().default('expert'),
    sdk: sdkTypeEnum('sdk').notNull().default('vercel_ai'),
  ...
  (table) => [
    ...
    index('agents_sdk_idx').on(table.sdk),
  ]
);

export const tools = publicSchema.table(
  'tools',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    type: toolTypeEnum('type').notNull().default('function'),

    // Tool configuration (schema, api definition, etc.)
    configuration: jsonb('configuration').notNull().default({}),

    isShared: integer('is_shared').default(1), // 0 = false, 1 = true

    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().notNull(),
  },
  (table) => [index('tools_type_idx').on(table.type)]
);

export const agentTools = publicSchema.table(
  'agent_tools',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const flows = publicSchema.table(
  'flows',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    name: text('name').notNull(),
    ...
    // The mediator agent that orchestrates this flow
    mediatorId: uuid('mediator_id').references(() => agents.id),

    triggerType: triggerTypeEnum('trigger_type').notNull().default('api'),
    outputMode: outputModeEnum('output_mode').notNull().default('stream'),
  ...
  (table) => [
    index('flows_sdk_idx').on(table.sdk),
    index('flows_mediator_idx').on(table.mediatorId),
  ]
);

export const flowAgents = publicSchema.table(
  'flow_agents',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    flowId: uuid('flow_id')
    ...
    role: text('role'), // Role of this agent in the flow

    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => [
    index('flow_agents_flow_idx').on(table.flowId),
    index('flow_agents_agent_idx').on(table.agentId),
  ]
);
...
export const knowledgeGraphRelations = relations(
  knowledgeGraph,
  ({ many }) => ({
...
export const retrievalSessionsRelations = relations(
  retrievalSessions,
  ({ many }) => ({
...
export const retrievalResultsRelations = relations(
  retrievalResults,
  ({ one }) => ({
...
export const embeddingShardsRelations = relations(
  embeddingShards,
  ({ many }) => ({
...
export const documentShardMappingsRelations = relations(
  documentShardMappings,
  ({ one }) => ({
    document: one(documents, {
